...
#define PIN_TRIG 1 // Пин триггер для дальномера
#define PIN_ECHO 0 // Пин эхо для дальномера
...
const int trigPin = 2; // Пин триггер для дальномера
const int echoPin = 3; // Пин эхо для дальномера
...
void setup() {
    ...
    //Init();// иниц. движков
    //runEngine(100,100,100,100); // Запуск двигателя
...
void Calculate() {
  ...
  */
    // float  h = CalculateHeight(ru.GetX(),ru.GetY()); // Считает высоту от поверхности с дальномера
    
     //CalculateEngines(0, ru); // Управляющее воздействие по высоте
    
    //float  h = CalculateHeight(0,0); // Код для теста высоты
...
// Считывание с дальномера (сторонний код)
int getDistSm(){
    int duration;
    float distance;
...
// Задаётся полётное задания
float getFlightTask(){
    unsigned long Time = millis() - START_OFFSET;
